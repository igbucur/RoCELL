...
// For function documentation, see Rcpp wrappers in src/RoCELL_RCpp_wrappers.cpp

// On Windows, this is necessary to link the code with the Rcpp libraries,
// which are (not sufficient and) separate from the MSYS2 libraries.
#ifdef CALL_ROCELL_FROM_R
...
arma::vec log_likelihood_gradient_scale_free(
  ...
  double term = Sigma_hat(0, 0) * p11 + 2 * Sigma_hat(0, 1) * p12 + 2 * Sigma_hat(0, 2) * p13 + 
    Sigma_hat(1, 1) * p22 + 2 * Sigma_hat(1, 2) * p23 + Sigma_hat(2, 2) * p33;
  ...
  double dsc24term = Sigma_hat(0, 0) * dsc24p11 + 2 * Sigma_hat(0, 1) * dsc24p12 + 
    2 * Sigma_hat(0, 2) * dsc24p13 + Sigma_hat(1, 1) * dsc24p22 +
    2 * Sigma_hat(1, 2) * dsc24p23 + Sigma_hat(2, 2) * dsc24p33;
  ...
  double dsc34term = Sigma_hat(0, 0) * dsc34p11 + 2 * Sigma_hat(0, 1) * dsc34p12 + 
    2 * Sigma_hat(0, 2) * dsc34p13 + Sigma_hat(1, 1) * dsc34p22 + 2 * Sigma_hat(1, 2) * dsc34p23;
  ...
  double dk1term = Sigma_hat(0, 0) * dk1p11 + 2 * Sigma_hat(0, 1) * dk1p12 + 2 * Sigma_hat(0, 2) * dk1p13;
  ...
  double dk2term = 2 * Sigma_hat(0, 1) * dk2p12 + Sigma_hat(1, 1) * dk2p22 + 2 * Sigma_hat(1, 2) * dk2p23;
  ...
  double dk3term = 2 * Sigma_hat(0, 2) * dk3p13 + 2 * Sigma_hat(1, 2) * dk3p23 + Sigma_hat(2, 2) * dk3p33;
  
  grad[0] = - (Sigma_hat(0, 0) * dsb21p11 + 2 * Sigma_hat(0, 1) * dsb21p12 + 2 * Sigma_hat(0, 2) * dsb21p13) * N / (2 * den);
  grad[1] = - (Sigma_hat(0, 0) * dsb31p11 + 2 * Sigma_hat(0, 1) * dsb31p12 + 2 * Sigma_hat(0, 2) * dsb31p13) * N / (2 * den);
  grad[2] = - (2 * Sigma_hat(0, 1) * dsb32p12 + Sigma_hat(1, 1) * dsb32p22 + 2 * Sigma_hat(1, 2) * dsb32p23) * N / (2 * den);
...
arma::mat log_likelihood_hessian_scale_free(
  ...
  double dsb21term = Sigma_hat(0, 0) * dsb21p11  + 2 * Sigma_hat(0, 1) * dsb21p12 + 2 * Sigma_hat(0, 2) * dsb21p13;
  ...
  double dsb21sc24term = Sigma_hat(0, 0) * dsb21sc24p11 + 2 * Sigma_hat(0, 1) * dsb21sc24p12 + 2 * Sigma_hat(0, 2) * dsb21sc24p13;
  ...
  double dsb21sc34term = Sigma_hat(0, 0) * dsb21sc34p11 + 2 * Sigma_hat(0, 1) * dsb21sc34p12 + 2 * Sigma_hat(0, 2) * dsb21sc34p13;
  
  // derivative wrt sb21 k1
  double dsb21k1p11 = dsb21p11 / k1;
  double dsb21k1p12 = dsb21p12 / (2 * k1);
  double dsb21k1p13 = dsb21p13 / (2 * k1);
  double dsb21k1term = Sigma_hat(0, 0) * dsb21k1p11 + 2 * Sigma_hat(0, 1) * dsb21k1p12 + 2 * Sigma_hat(0, 2) * dsb21k1p13;
  
  // derivative wrt sb21 k2
  double dsb21k2term = Sigma_hat(0, 1) * dsb21p12 / k2;
  
  // derivative wrt sb21 k3
  double dsb21k3term = Sigma_hat(0, 2) * dsb21p13 / k3;
  ...
  double dsb31term = Sigma_hat(0, 0) * dsb31p11  + 2 * Sigma_hat(0, 1) * dsb31p12 + 2 * Sigma_hat(0, 2) * dsb31p13;
  ...
  double dsb31sc24term = Sigma_hat(0, 0) * dsb31sc24p11 + 2 * Sigma_hat(0, 1) * dsb31sc24p12 + 2 * Sigma_hat(0, 2) * dsb31sc24p13;
  
  // derivative wrt sb31 sc34
  double dsb31sc34p11 = - 2 * k1 * sc24 * sb21;
  double dsb31sc34p12 = sqrt(k1 * k2) * sc24;
  
  double dsb31sc34term = Sigma_hat(0, 0) * dsb31sc34p11 + 2 * Sigma_hat(0, 1) * dsb31sc34p12;
  ...
  double dsb31k1term = Sigma_hat(0, 0) * dsb31k1p11 + 2 * Sigma_hat(0, 1) * dsb31k1p12 + 2 * Sigma_hat(0, 2) * dsb31k1p13;
  
  // derivative wrt sb31 k2
  double dsb31k2term = Sigma_hat(0, 1) * dsb31p12 / k2;
  
  // derivative wrt sb31 k3
  double dsb31k3term = Sigma_hat(0, 2) * dsb31p13 / k3;
  ...
  double dsb32term = 2 * Sigma_hat(0, 1) * dsb32p12  + Sigma_hat(1, 1) * dsb32p22 + 2 * Sigma_hat(1, 2) * dsb32p23;
  ...
  double dsb32sc24term = 2 * Sigma_hat(0, 1) * dsb32sc24p12 + Sigma_hat(1, 1) * dsb32sc24p22 + 2 * Sigma_hat(1, 2) * dsb32sc24p23;
  
  // derivative wrt sb32 sc34
  double dsb32sc34p12 = - sqrt(k1 * k2) * sc24 * sb21;
  double dsb32sc34p22 = 2 * k2 * sc24;
  
  double dsb32sc34term = 2 * Sigma_hat(0, 1) * dsb32sc34p12 + Sigma_hat(1, 1) * dsb32sc34p22;
  
  // derivative wrt sb32 k1
  double dsb32k1term = Sigma_hat(0, 1) * dsb32p12 / k1;
  ...
  double dsb32k2term = 2 * Sigma_hat(0, 1) * dsb32k2p12 + Sigma_hat(1, 1) * dsb32k2p22 + 2 * Sigma_hat(1, 2) * dsb32k2p23;
  
  // derivative wrt sb32 k3
  double dsb32k3term = Sigma_hat(1, 2) * dsb32p23 / k3;
  ...
  double dsc24sc24term = Sigma_hat(0, 0) * dsc24sc24p11 + 2 * Sigma_hat(0, 1) * dsc24sc24p12 + 2 * Sigma_hat(0, 2) * dsc24sc24p13 + 
    Sigma_hat(1, 1) * dsc24sc24p22 + 2 * Sigma_hat(1, 2) * dsc24sc24p23 + Sigma_hat(2, 2) * dsc24sc24p33;
  ...
  double dsc24sc34term = Sigma_hat(0, 0) * dsc24sc34p11 + 2 * Sigma_hat(0, 1) * dsc24sc34p12 + 
    2 * Sigma_hat(0, 2) * dsc24sc34p13 + Sigma_hat(1, 1) * dsc24sc34p22 + 2 * Sigma_hat(1, 2) * dsc24sc34p23;
  
  // derivative wrt sc24 k1
  double dsc24k1term = (Sigma_hat(0, 0) * dsc24p11 + Sigma_hat(0, 1) * dsc24p12 + Sigma_hat(0, 2) * dsc24p13) / k1;
  
  // derivative wrt sc24 k2
  double dsc24k2term = (Sigma_hat(1, 1) * dsc24p22 + Sigma_hat(0, 1) * dsc24p12 + Sigma_hat(1, 2) * dsc24p23) / k2;
  
  // derivative wrt sc24 k3
  double dsc24k3term = (Sigma_hat(2, 2) * dsc24p33 + Sigma_hat(1, 2) * dsc24p23 + Sigma_hat(0, 2) * dsc24p13) / k3;
  ...
  double dsc34sc34term = Sigma_hat(0, 0) * dsc34sc34p11 + 2 * Sigma_hat(0, 1) * dsc34sc34p12 + Sigma_hat(1, 1) * dsc34sc34p22;
  
  // derivative wrt sc34 k1
  double dsc34k1term = (Sigma_hat(0, 0) * dsc34p11 + Sigma_hat(0, 1) * dsc34p12 + Sigma_hat(0, 2) * dsc34p13) / k1;
  
  // derivative wrt sc34 k2
  double dsc34k2term = (Sigma_hat(1, 1) * dsc34p22 + Sigma_hat(0, 1) * dsc34p12 + Sigma_hat(1, 2) * dsc34p23) / k2;
  
  // derivative wrt sc34 k3
  double dsc34k3term = (Sigma_hat(1, 2) * dsc34p23 + Sigma_hat(0, 2) * dsc34p13) / k3;
  ...
  double dk1k1term = 2 * Sigma_hat(0, 1) * dk1k1p12 + 2 * Sigma_hat(0, 2) * dk1k1p13;
  
  // derivative wrt k1 k2
  double dk1k2term = Sigma_hat(0, 1) * dk1p12 / k2;
  
  // derivative wrt k1 k3
  double dk1k3term = Sigma_hat(0, 2) * dk1p13 / k3;
  ...
  double dk2k2term = 2 * Sigma_hat(0, 1) * dk2k2p12 + 2 * Sigma_hat(1, 2) * dk2k2p23;
  
  // derivative wrt k2 k3
  double dk2k3term = Sigma_hat(1, 2) * dk2p23 / k3;
  ...
  double dk3k3term = 2 * Sigma_hat(0, 2) * dk3k3p13 + 2 * Sigma_hat(1, 2) * dk3k3p23;
  
  // grad[0] = - (Sigma_hat(0, 0) * dsb21p11 + 2 * Sigma_hat(0, 1) * dsb21p12 + 2 * Sigma_hat(0, 2) * dsb21p13) * N / (2 * den);
  
  hess(0, 0) = - Sigma_hat(0, 0) * dsb21sb21p11 * N / (2 * den);
  hess(0, 1) = hess(1, 0) = - Sigma_hat(0, 0) * dsb21sb31p11 * N / (2 * den);
  hess(0, 2) = hess(2, 0) = - 2 * Sigma_hat(0, 1) * dsb21sb32p12 * N / (2 * den);
  ...
  hess(1, 1) = - Sigma_hat(0, 0) * dsb31sb31p11 * N / (2 * den);
  hess(1, 2) = hess(2, 1) = - 2 * Sigma_hat(0, 1) * dsb31sb32p12 * N / (2 * den);
  ...
  hess(2, 2) = hess(2, 2) = - Sigma_hat(1, 1) * dsb32sb32p22 * N / (2 * den);
...
arma::mat log_likelihood_hessian_scale_free_confounders(
  ...
  arma::vec grad = log_likelihood_gradient_scale_free(Sigma_hat, N, sb21, sb31, sb32, sc24, sc34, v1, v2, v3);
  arma::mat orig = log_likelihood_hessian_scale_free(Sigma_hat, N, sb21, sb31, sb32, sc24, sc34, v1, v2, v3);
...
double log_posterior_RoCELL(
    arma::mat Sigma_hat, unsigned N, double sb21, double sb31, double sb32,
    double sc24, double sc34, double v1, double v2, double v3,
    double slab_precision, double spike_precision, double w32) {
  
  arma::mat sf_hess = log_likelihood_hessian_scale_free(Sigma_hat, 1, sb21, sb31, sb32, sc24, sc34, v1, v2, v3);
  ...
  return - sf_ld +
    log_spike_and_slab_scale_free(sb21, 0.5, slab_precision, spike_precision) +
    log_spike_and_slab_scale_free(sb31, 0.5, slab_precision, spike_precision) +
    log_spike_and_slab_scale_free(sb32, w32, slab_precision, spike_precision) +
