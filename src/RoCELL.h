...
inline void print() {
#ifdef CALL_ROCELL_FROM_R
  std::cout << "Rcpp enabled" << std::endl;
#else
  std::cout << "Rcpp not enabled" << std::endl;
#endif
}
...
// 
// Eigen::MatrixXd two_times(Eigen::MatrixXd x);

arma::mat two_times_arma(arma::mat x) {
  return 2 * x;
};
...
double log_posterior_RoCELL(
  ...
  arma::mat hess = log_likelihood_hessian_scale_free_confounders(
    sncsm, N, sb21, sb31, sb32, sc24, sc34, v1, v2, v3);
  
  arma::mat neg_hess_observed(6, 6);
  
  neg_hess_observed.submat(0, 0, 2, 2) = - hess.submat(0, 0, 2, 2);
  neg_hess_observed.submat(3, 0, 5, 2) = - hess.submat(5, 0, 7, 2);
  neg_hess_observed.submat(0, 3, 2, 5) = - hess.submat(0, 5, 2, 7);
  neg_hess_observed.submat(3, 3, 5, 5) = - hess.submat(5, 5, 7, 7);
  
  double ld = 0;
  arma::mat U = chol(neg_hess_observed, "lower");
  for (unsigned i = 0; i < U.n_rows; ++i) ld += log(U(i,i));
  ld *= 2;
  
  return (- ld / 2.0 +
          log_spike_and_slab(sb21, v1, v2, 0.5, slab_precision, spike_precision) +
          log_spike_and_slab(sb31, v1, v3, 0.5, slab_precision, spike_precision) +
          log_spike_and_slab(sb32, v2, v3, w32, slab_precision, spike_precision) +
